...
void th_lifeUpdate(th_LifeSphere* o,float dt)
{
    ...
    for (uint i = 0; i <o->num_used;i++)
    {
        ...
        if (o->lifes[i] < 0)
        {
         ...
         if (o->lifes[i] > 0 && fn_distance2(target,entities[i].aabb.position) <= 200*200 && o->levelstate->player->hp < 200 )
        {
            o->lifes[i] = -30000;

            th_incrementPlayerHealth(o->levelstate->player,5);
            th_setGameGlow(fn_createVec3(0,1,1),0.2);
            //printf("%s\n","GOT GEM" );
        }
