...
float raymarch(vec3 worldspaceRay,vec3 worldSpaceDirection,samplerCubeArray mp,float cubeindex,vec3 cubepos,out vec3 result,out float dist_to_hit,int iterations,int steps)
{
  ...
  float depth = 0;// texture(mp, vec4(worldSpaceDirection*vec3(-1,1,1),cubeindex)).r;
...
float synflag = 0;
vec4 traceCubes(float depthval,vec3 position,vec3 normal,vec3 refnormal,float roughness,vec3 viewDir,float metal,int quality,vec3 F0)
{
  float alpha = 1;
  float index = -1;
  ...
  if (count ==0)
  {
    ...
    return vec4(0,0,0,0);//vec4(lookup_fast.xyz,fastNdotL);
  }
  ...
  //  L = L*(1-metalfactor) + metalfactor*-refnormal;
    ...
    // float NoL = NdotL;
    // float NoH = max( dot( normal, normalize( -L + -viewDir ) ),0.0 );
    // float VoH = max( dot( viewDir, H ),0.0 );
    //
    // float G = GeometrySmith ( normal,-viewDir,-L,roughness );
    // //
     // float Fc = pow( 1 - max( dot( viewDir, normalize( -L + viewDir ) ),0.0 ), 5 );
     // vec3  F = (1 - Fc) * F0 + Fc;
    // float brdfweight =   G * VoH / max( NoV * NoH,0.00001) ;//(G * VoH) / (NoH * NoV);
    // brdfweight *= when_gt( NoV * NoH,0.0);

    vec3 F = SchlickFresnel(F0, dot( -viewDir, normalize( -L + -viewDir ) ));
        float G1 = SmithGGXMasking(normalize( -L + viewDir ),-L,viewDir, roughness*roughness);
        float G2 = SmithGGXMaskingShadowing(normalize( -L + viewDir ),-L,viewDir, roughness*roughness);


    vec3 reflectance = F *(G2 / G1);
    //reflectance *= when_gt(G1,0.0);
    accum += lookup2.xyz*reflectance;//F*brdfweight;//*NdotL*when_gt(NdotL,0);//*(NdotL)*when_gt(NdotL,0);
    total_weight += 1;//brdfweight;//NdotL*when_gt(NdotL,0);
    #else
...
vec3 albedo     = pow(a.rgb,vec3(2.2));//pow(texture(albedoMap, outTexcoord).rgb, vec3(2.2));
  ...
  {
    fragColor =  traceCubes( depthval, position, normal, refnormal, roughness,viewDir,metallic,quality,F0);//*vec4(val,1);
  }
}
