...
vec4 traceCubes(float depthval,vec3 position,vec3 normal,vec3 refnormal,float roughness,vec3 viewDir,float metal,int quality,vec3 F0)
{
  ...
  //  vec3 F = SchlickFresnel(F0, dot(viewDir, H));
    // float G1 = SmithGGXMasking(H,-L,viewDir, roughness);
    // float G2 = SmithGGXMaskingShadowing(H, -L, viewDir,roughness);

    accum += lookup2.xyz*F * brdfweight;//*NdotL*when_gt(NdotL,0);//*(NdotL)*when_gt(NdotL,0);
