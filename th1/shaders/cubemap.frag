...
const float MAX_TRACE = 3000.0;

layout (std140 , binding = 0) uniform EnvBoxes
{
  vec3 EnvBoxPos[32];// 0
  vec3 EnvBoxMin[32];// size*16
  vec3 EnvBoxMax[32];//size * 16*2
...
vec4 traceCubes(float depthval,vec3 position,vec3 normal,vec3 refnormal,float roughness,vec3 viewDir,float metal,int quality,vec3 F0,vec3 brdf)
{
  float alpha = 1;
  float index = -1;
  ...
  for (int i = 0 ; i < count;i++)
  {
    ...
    float dist_tex = texture(reflectionsDepth, vec4(normalize(offsetpos - EnvBoxPos[key])*vec3(-1,1,1),key)).r;
    float cond = when_gt((dist_tex*dist_tex - distance_squared(offsetpos,EnvBoxPos[key])),-100);
  ...
  lookup2 = lookup2*h2 + textureLod(atmosphereCube,(-L*vec3(-1,1,1)),roughness*0.0)*(1-h2);
  ...
  alpha = fastNdotL;
  #else
  float h = raymarch(position,refnormal,reflectionsDepth,index,EnvBoxPos[int(index)],l,dist_to_hit);
...
//  fragColor = vec4(normal*0.5 + 0.5,uintBitsToFloat(packHalf2x16(vec2(0,1))));
  ...
  {
    fragColor =  traceCubes( depthval, position, normal, refnormal, roughness,viewDir,metallic,quality,F0,val);
  }
}
