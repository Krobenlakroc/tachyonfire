...
vec3(-0.153523,0.0680838,0.152168),
  ...
  mat4 view = inverse(viewMatrixInv);
  ...
  vec3 tangent = normalize(noise - center_normal * dot(noise, center_normal));
	...
	const float fudge_factor_l0 = 1;//2.0;
	const float fudge_factor_l1 = 1;//10.0;
  ...
  c8 = 0;
  //unroll
  vec3 world_dir = -normalize(vec3(0,0.4,-1));
	for( int i=0; i<num_samples; ++i )
	{
    float hit = 0;
    float steps = 0.0;
    vec3 sample_world;
    vec4 sfv;//view*vec4(reflect(normalize(center_world - eyePos),wn),0)).xyz;//
    vec3 dir= (view*vec4(-normalize(vec3(0,0.4,-1)),0)).xyz;
    vec2 uv;
    for (int j = 0;j < 12;j++)
    {
    //  vec3 point = dir;//TBN * points[i]; // from tangent to view-space
      vec3 point = center_pos + dir * steps;
      steps += 1;
      ...
      offset.xyz = offset.xyz * 0.5 + 0.5; // transform to range 0.0 - 1.0
       sfv = ViewPosFromDepth(texture(depthTex,offset.xy).r);
      vec3 sample_pos = sfv.xyz;
      uv = offset.xy;
        if (point.z < sample_pos.z)
        {
          hit =1;//*dot(dir,);
          break;
        }
    ...
    sample_world = worldfromview(sfv);
    float rangeCheck = when_lt(distance(center_world, sample_world),occlusion_max_distance);
//when_lt(abs(center_pos.z - sample_pos.z),occlusion_max_distance);//smoothstep(0.0, 1.0, occlusion_max_distance / abs(center_pos.z - sample_pos.z));
  ...
  // float dist = distance(sample_world ,center_world);
  // float dp = dot(wn, worldnormal);
  // float attenuation = 1-clamp(dist * (1/occlusion_max_distance),0,1);
  //  attenuation = attenuation*attenuation * step(0.1, dp);
  vec2 dCoords = smoothstep(0.2, 0.6, abs(vec2(0.5, 0.5) - uv.xy));
  float screenEdgefactor = clamp(1.0 - (dCoords.x + dCoords.y), 0.0, 1.0);
  screenEdgefactor *= -dir.z;
//  hit *= when_gt(dot(normalize(sfv.xyz - center_pos),dir),0)*clamp(screenEdgefactor,0,1);
		 occlusion_sh2 += vec4(hit*rangeCheck*screenEdgefactor);//(hit* rangeCheck)*sh2_weight*vec4(worldnormal,1).yzxw;
  ...
  // c8 *= (4*3.14159)/(2*num_samples);
   //occlusion_sh2 *= (4*3.14159)/(2*num_samples);
