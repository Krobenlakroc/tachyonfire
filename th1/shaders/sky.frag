...
float cloudShadowTransmittance(vec3 pos, vec3 sunDir,
                                float cloudBase, float cloudTop) {
...
//         optDepth += density * stepLen * 1e-4; // scale to plausible optical depth
        ...
        optDepth += max(cloudDensity,0.0) * stepLen * 0.001;
    }
...
//     return vec4(cloudColor, cloudAlpha);
    ...
    vec3  sunContrib = sun_color * sun_intensity * phase * shadowT * beerP * sunZenith * 0.8;
    ...
    vec3  ambientSky  = normalize(rayleigh) * sun_intensity * 0.04 * (0.5 + 0.5 * sunZenith);

    // Cloud base darkening: underlit region gets a warmer, darker hue.
    float baseDark   = 1.0 - shadowT * 0.7;
    vec3  baseColor  = shadowT*vec3(1,1,1);//vec3(0.6, 0.55, 0.5) * baseDark;
...
void main(void)
{
    ...
    if (cloud_enable == 2.0)
    {
        vec4 clouds = renderClouds(vec3(0, 6372e3, 0), dir, uSunPos);
